...
def run_backtest_engine(days_back=14):
    """
    Executes a historical backtest against factual, finalized MLB data.
    Defaults to the last 14 days of games.
    """
    ...
    # Hydrate probablePitcher pulls the actual starters for historical math calculation
    ...
    for date_data in response.get('dates', []):
        for game in date_data.get('games', []):
            ...
            # Fetch current dynamic multipliers
            ...
            # Monte Carlo Processing (2,500 iterations per game for backtest speed)
            iterations = 2500
            ...
            # Brier Score Formulation: (Forecasted Probability - Empirical Outcome)^2
            brier_score_sum += (home_prob - actual_home_win) ** 2
            
            # Root Mean Square Error (RMSE) formulation
            ...
            # ROI Simulation (Assuming a 1 unit flat wager at standardized -110 odds)
            if is_correct:
                units_won += 0.909
            else:
                units_won -= 1.000
    ...
    print(f"Total Matchups Verified:  {total_games}")
