...
def audit_props_slate():
    ...
    # Query active game slates from forecasts
    games = c.execute("""
        SELECT DISTINCT game_pk FROM Batter_Hit_Forecasts
        UNION
        SELECT DISTINCT game_pk FROM Pitcher_K_Forecasts
    """).fetchall()
    ...
    for g in games:
        ...
        # Check if game is official/final
        status_url = f"https://statsapi.mlb.com/api/v1/game/{pk}/linescore"
        try:
            s_res = requests.get(status_url, timeout=5)
            s_data = s_res.json()
            is_final = any(x in s_data.get("inningState", "").lower() for x in ["final", "end"]) or s_data.get("currentInning", 0) >= 9
        except Exception:
            is_final = False
        ...
        for b in batter_rows:
            ...
            # Locate player in official MLB boxscore
            actual_hits = None
            actual_ab = 0
            actual_pa = 0
            ...
            if actual_hits is not None and (actual_pa > 0 or is_final):
                ...
                # Empirical Bayes Adaptation: w = N / (N + 80)
                ...
                n = b_mod["sample_pa"] + actual_pa
                old_mod = b_mod["contact_modifier"]

                # Shift contact modifier based on expected vs actual contact
        ...
        # -------------------------------------------------------------
        # 2. AUDIT PITCHER STRIKEOUT FORECASTS
        # -------------------------------------------------------------
        ...
        for p in pitcher_rows:
            ...
            if actual_k is not None and (act_pitches > 0 or is_final):
                ...
                # EWMA Modifier Adaptation: alpha = 1 / sqrt(N + 1)
                sp_row = c.execute("SELECT k_modifier, sample_starts, pitches_per_bf FROM Pitcher_Stats WHERE pitcher_name = ?", (sp_name,)).fetchone()
                if sp_row:
                    ...
                    c.execute("""
                        ...
                        WHERE pitcher_name = ?
                    """, (new_k_mod, new_p_bf, sp_name))

    conn.commit()
    conn.close()
    print("[SUCCESS] Factual post-mortem prop audit complete. Batter & pitcher modifiers updated.")
