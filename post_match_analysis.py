...
def update_dynamic_weights(cursor, team_name, predicted_runs, actual_runs, is_offense=True):
    ...
    if is_offense:
        # Offense scored more than expected -> boost modifier; scored less -> lower modifier
        ...
        # Pitching/Defense allowed more runs than expected -> increase penalty modifier; allowed fewer -> lower modifier
...
def run_post_match_analysis():
    ...
    # Check yesterday's and today's dates to capture completed evening games
    dates_to_check = [
    ...
    for date_str in dates_to_check:
        ...
        for date_data in response.get('dates', []):
            for game in date_data.get('games', []):
                ...
                if status == 'Final':
                    # Verify if game was already processed to prevent duplicate weight application
                    ...
                    if forecast:
                        ...
                        # Execute dynamic weight calibration if predicted run baselines exist
                        if pred_home_runs is not None and pred_away_runs is not None:
                            # Home Team Offense & Away Team Pitching calibration
                            update_dynamic_weights(cursor, home_team, pred_home_runs, home_score, is_offense=True)
                            update_dynamic_weights(away_team, pred_home_runs, home_score, is_offense=False)
                            
                            # Away Team Offense & Home Team Pitching calibration
                            update_dynamic_weights(cursor, away_team, pred_away_runs, away_score, is_offense=True)
                            update_dynamic_weights(home_team, pred_away_runs, away_score, is_offense=False)
