...
def run_unified_post_mortem():
    ...
    # Scope audit strictly to active games in Daily_Lineups, or active props
    game_pks = []
    if "Daily_Lineups" in tables:
        rows = c.execute("SELECT DISTINCT game_pk FROM Daily_Lineups WHERE game_pk IS NOT NULL").fetchall()
        game_pks = [r["game_pk"] for r in rows]

    if not game_pks:
        for tbl in ["Pitcher_K_Forecasts", "Batter_Hit_Forecasts"]:
            if tbl in tables:
                rows = c.execute(f"SELECT DISTINCT game_pk FROM {tbl} WHERE game_pk IS NOT NULL").fetchall()
                for r in rows:
                    if r["game_pk"] not in game_pks:
                        game_pks.append(r["game_pk"])

    if not game_pks:
        print("[POST-MORTEM] No active games found in Daily_Lineups or props tables to evaluate.", flush=True)
        conn.close()
        return

    print(f"[POST-MORTEM] Found {len(game_pks)} active games to audit. Beginning live verification...", flush=True)
    ...
    # Check Pitcher_Stats columns
    ps_cols = [r[1] for r in c.execute("PRAGMA table_info(Pitcher_Stats)").fetchall()] if "Pitcher_Stats" in tables else []
    p_name_col = "pitcher_name" if "pitcher_name" in ps_cols else ("clean_name" if "clean_name" in ps_cols else "last_name")

    for idx, pk in enumerate(game_pks, 1):
        print(f"--> [{idx}/{len(game_pks)}] Auditing GamePK {pk}...", flush=True)
        line_url = f"https://statsapi.mlb.com/api/v1/game/{pk}/linescore"
        box_url = f"https://statsapi.mlb.com/api/v1/game/{pk}/boxscore"

        try:
            line_res = requests.get(line_url, timeout=(3.0, 5.0))
            if line_res.status_code != 200:
                print(f"    Linescore HTTP {line_res.status_code} - skipping.", flush=True)
                continue
            line = line_res.json()
        except requests.RequestException as e:
            print(f"    Linescore network timeout/error: {e}", flush=True)
            continue

        innings = line.get("innings", [])
        if len(innings) < 5:
            print("    Game has not completed 5 innings yet.", flush=True)
            continue
        ...
        # 1. Audit Full Game ML & F5
        if "Model_Forecasts" in tables:
            mf = c.execute("SELECT * FROM Model_Forecasts WHERE game_pk = ?", (pk,)).fetchone()
            if mf:
                p_home = float(mf["prob_home_win"] or 0.50) if "prob_home_win" in mf.keys() else float(mf["home_prob"] or 0.50)
        ...
        # 2. Audit Pitchers & Batters via Boxscore
        try:
            box_res = requests.get(box_url, timeout=(3.0, 5.0))
            ...
            print(f"    Boxscore network timeout/error: {e}", flush=True)

    if f5_run_biases and "Telemetry_Drift" in tables:
    ...
    conn.commit()
    conn.close()
    print("[COMPLETE] Unified audit finished.", flush=True)
