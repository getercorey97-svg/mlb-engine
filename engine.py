...
def run_ultimate_monte_carlo():
    ...
    for game in games:
        (game_pk, away, home, away_pitcher, home_pitcher, air_density, umpire_multiplier,
         away_framing, away_fatigue, home_framing, home_fatigue, away_bio_penalty, home_bio_penalty) = game
        
        # Calculate Density Multiplier with Failsafe Clamp
        ...
        # APPLY SOTA BIOLOGICAL JET LAG PENALTIES
        away_lambda_total = max(0.1, away_lambda_total - away_bio_penalty)
        home_lambda_total = max(0.1, home_lambda_total - home_bio_penalty)
