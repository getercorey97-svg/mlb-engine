...
def run_ultimate_monte_carlo():
    print("Initializing Ultimate Monte Carlo Engine with Dynamic Modifiers...")
    print("Synthesizing Statcast, Offense, Bullpens, Dynamic Modifiers, and Park Factors...")
    
    conn = sqlite3.connect('mlb_engine.db')
    cursor = conn.cursor()

    # Ensure required columns exist in Model_Forecasts for post-match delta calculation
    for col in ["predicted_home_runs REAL", "predicted_away_runs REAL"]:
    ...
    for game in games:
        ...
        # 1. Fetch Dynamic Feedback Modifiers (Baseline is 1.0)
        ...
        # Pitchers faced: Away batters face Home starter; Home batters face Away starter
        ...
        # Away offense adjusted by dynamic weight vs Home pitching adjusted by dynamic weight
        ...
        # Starters (~6 innings: 0.66), Bullpens (~3 innings: 0.33)
        ...
        # 4. Execute Stochastic Monte Carlo Simulation (10,000 Iterations)
        iterations = 10000
        
        away_sims = np.random.poisson(away_lambda_total, iterations)
        home_sims = np.random.poisson(home_lambda_total, iterations)
        
        # Resolve Extra Innings (Ghost runner: ~1.1 runs expected)
        ties = away_sims == home_sims
        while np.any(ties):
            away_extra = np.random.poisson((adj_home_bullpen / 3) + 1.1, np.sum(ties))
            home_extra = np.random.poisson((adj_away_bullpen / 3) + 1.1, np.sum(ties))
