...
def fetch_pregame_pressure_drop(team_name, date_str):
    ...
    params = {
        ...
        "start_date": date_str, "end_date": date_str, "hourly": "surface_pressure"
    }
    try:
...
def build_correlation_dataset(conn):
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS Feature_Correlations (
            feature_name TEXT PRIMARY KEY, outcome_r REAL, run_total_r REAL,
            error_delta_r REAL, combined_r_squared REAL, status TEXT, last_analyzed TEXT
        )
    ''')
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS Esoteric_Signals (
            game_pk INTEGER PRIMARY KEY, geomagnetic_kp REAL DEFAULT 2.0,
            solar_xray_flux REAL DEFAULT 1.0, home_media_pressure INTEGER DEFAULT 0,
            home_media_tone REAL DEFAULT 0.0, roster_birthday_active INTEGER DEFAULT 0,
            captured_at TEXT
        )
    ''')
    conn.commit()
